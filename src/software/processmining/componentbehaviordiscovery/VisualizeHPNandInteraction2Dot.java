...
public class VisualizeHPNandInteraction2Dot {
	...
	public static DotCluster visualizeHPN2Dot(HierarchicalMultiinstancePetriNet hpn, String currentName, final DotCluster dot, 
	...
	{	
		...
		final HashMap<XEventClass, HierarchicalMultiinstancePetriNet> XEventClass2hpn =hpn.getXEventClass2hpn();
		...
		for (final Transition t : pn.getTransitions()) 
		{
			...
			else// normal transition
			{
				...
				if (nestedFlag==1) // for normal transitions
				{
...
//			pDot = new LocalDotPlace();
		...
		if(XEventClass2hpn.keySet().size()!=0)
		{
			for(XEventClass xevent: XEventClass2hpn.keySet())
			{
				...
				{
					String xeventnamestr = xevent.toString();
					if (t.getLabel().equals(xeventnamestr))
					{
						...
						tempEdge.setOption("label", "1");
						Nested_cluster.setOption("label", ActivityNameMethodClassPackage(xevent.toString()));
						
					}
					else if(xeventnamestr.contains("_") && t.getLabel().equals(xeventnamestr.substring(0, xeventnamestr.indexOf("_")))) {
						//add the arc from nested transition to the cluster
						List<DotNode> nodeList = Nested_cluster.getNodes();
						...
						//tempEdge.setOption("label", "Call");// label
						tempEdge.setOption("fontcolor", "blue");
						tempEdge.setOption("label", "+");
						Nested_cluster.setOption("label", ActivityNameMethodClassPackage(xeventnamestr.substring(0, xeventnamestr.indexOf("_"))));
					}
				}
			}
		}	
...
//					
//					//变迁的名字是否在嵌套变迁集合里
//					if( t.getLabel().equals(xevent.toString().substring(0, xevent.toString().indexOf('_'))))
//					{
//						nestedFlag=1;
//						break;
//					}
////					else if(t.getLabel().equals(xevent.toString())) {
////						nestedFlag=2;
////						break;
////					}
//				}
...
//				DotCluster Nested_cluster =dot.addCluster();
//				//
//				Nested_cluster.setOption("label", ActivityNameMethodClassPackage(xevent.toString().substring(0, xevent.toString().indexOf('_'))));
//				Nested_cluster.setOption("penwidth", "2.0");
//				Nested_cluster.setOption("style","dashed");   //dashed 虚线
//				Nested_cluster.setOption("color","blue");
//				
//				visualizeHPN2Dot(XEventClass2hpn.get(xevent),ActivityNameMethodClassPackage(xevent.toString().substring(0, xevent.toString().indexOf('_'))), Nested_cluster, interactionMethod2InteractionCluster);// recursion, c2Inter
//				
//				// add arc from nested transitions to sub-cluster
//				for(Transition t : pn.getTransitions())
//				{
...
//						tempEdge.setOption("fontcolor", "blue");
//						tempEdge.setOption("label", "1");
//						tempEdge.setOption("labelcolor", "black");
//						
//					}
//				}
//			}
